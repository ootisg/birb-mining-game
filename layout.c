	...
	if (json_obj) {
		...
		while (curr) {
			...
			if (!strcmp (type_str, "rect")) {
				...
				float wx;
				float wy;
				for (wy = 0; wy + new_height <= parent_bounds->y + parent_bounds->height + .00001; wy += new_height) {
					for (wx = 0; wx + new_width <= parent_bounds->x + parent_bounds->width + .00001; wx += new_width) {
